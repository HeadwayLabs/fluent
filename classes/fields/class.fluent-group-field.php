...
add_action('fluent/options/field/group/render', array('Fluent_Group_Field', 'render'), 1, 2);
add_filter('fluent/options/field/group/schema', array('Fluent_Group_Field', 'schema'), 1, 2);
add_filter('fluent/options/field/group/default', array('Fluent_Group_Field', 'default_value'), 1, 2);
add_action('fluent/options/field/group/enqueue', array('Fluent_Group_Field', 'enqueue'));

/**
 * Fluent_Group_Field repeatable, nestable groups of fields.
 */
class Fluent_Group_Field extends Fluent_Field{
    ...
    public static function field_data(){
        ...
        return array(
            ...
            'layout' => 'horizontal',
            'show_headers' => true,
            'add_row_text' => __('Add Row', $self->domain)
        );
    }
    ...
    public static function enqueue( $data = array(), $field_data = array() ){
        foreach($data['fields'] as $k => $field){
            if(isset(Fluent_Options::$field_types[$field['type']])){
                do_action('fluent/options/field/'.$field['type'].'/enqueue', $field, Fluent_Options::$field_types[$field['type']]);
            }
        }
    }
     ...
     * @since 1.0.0
     *
     * @uses Fluent_Base to generate guids for array values
     *
                                    ...
                                    echo '<tr id="field-'.$data['id'].'-'.$value_key.'">';
                                        ...
                                        foreach($data['fields'] as $key => $field){
                                            ...
                                            echo '<td id="field-'.$key.'" class="field-type-'.$field['type'].'">';
                                                if(!isset(Fluent_Options::$field_types[$field['type']])){
                                                    echo '<p><strong>'.__('Field type not registered!',$object->domain).'</strong></p>';
                                                }else{
                                                    do_action('fluent/options/field/'.$field['type'].'/render', $field, $object);
                                                }
