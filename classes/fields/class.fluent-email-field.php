...
add_action('fluent/options/field/email/render', array('Fluent_Email_Field', 'render'), 1, 2);

/**
 * Fluent_Email_Field simple text field.
 */
class Fluent_Email_Field extends Fluent_Field{
    ...
    public static function render($data = array(), $object){
        ...
        echo '<input type="email" name="'.$data['option_name'].'['.$data['name'].']" id="'.$data['id'].'" value="'.$data['value'].'" class="'.implode(' ', $data['classes']).'" placeholder="'.$data['placeholder'].'" />';   
    }
